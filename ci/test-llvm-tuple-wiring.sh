...
node --test --test-reporter=tap --test-name-pattern='tuple' ci/release/prepare_bootstrap_inputs.test.mjs > "$work/consumer-green.log" 2>&1
python3 - <<'PY'
...
set +e
node --test --test-reporter=tap --test-name-pattern='tuple' ci/release/prepare_bootstrap_inputs.test.mjs > "$work/consumer-cut.log" 2>&1
consumer_rc=$?
set -e
...
set +e
node --test --test-reporter=tap --test-name-pattern='tuple' ci/release/prepare_bootstrap_inputs.test.mjs > "$work/pin-cut.log" 2>&1
pin_rc=$?
set -e
test "$pin_rc" -ne 0
...
node --test --test-reporter=tap --test-name-pattern='tuple' ci/release/prepare_bootstrap_inputs.test.mjs > "$work/consumer-restored.log" 2>&1
