...
node --test --test-reporter=tap --test-name-pattern='^(?!ast ).*(tuple|depot|persistent)' ci/release/prepare_bootstrap_inputs.test.mjs > "$work/consumer-green.log" 2>&1
python3 - <<'PY'
...
s = p.read_text()
line = "mode: process.env.CJCJ_BOOTSTRAP_SOURCE || 'release',"
assert s.count(line) == 1
p.write_text(s.replace(line, "mode: process.env.CJCJ_BOOTSTRAP_SOURCE || 'depot',"))
PY
diff -u "$work/consumer.saved" "$consumer" > "$work/consumer-cut.diff" || test "$?" -eq 1
sha256sum "$consumer" > "$work/consumer-cut.sha256"
set +e
node --test --test-reporter=tap --test-name-pattern='^(?!ast ).*(tuple|depot|persistent)' ci/release/prepare_bootstrap_inputs.test.mjs > "$work/consumer-cut.log" 2>&1
consumer_rc=$?
set -e
test "$consumer_rc" -ne 0
grep -F 'ERR_ASSERTION' "$work/consumer-cut.log"
grep -Fx '# pass 4' "$work/consumer-cut.log"
grep -Fx '# fail 2' "$work/consumer-cut.log"
restore
python3 - <<'CUT'
from pathlib import Path
p = Path('ci/release/prepare_bootstrap_inputs.mjs')
...
set +e
node --test --test-reporter=tap --test-name-pattern='^(?!ast ).*(tuple|depot|persistent)' ci/release/prepare_bootstrap_inputs.test.mjs > "$work/pin-cut.log" 2>&1
pin_rc=$?
set -e
test "$pin_rc" -ne 0
grep -F 'ERR_ASSERTION' "$work/pin-cut.log"
grep -Fx '# pass 5' "$work/pin-cut.log"
grep -Fx '# fail 1' "$work/pin-cut.log"
restore
bash ci/test-llvm-tuple-layout.sh "$work/producer-restored" > "$work/producer-restored.log" 2>&1
node --test --test-reporter=tap --test-name-pattern='^(?!ast ).*(tuple|depot|persistent)' ci/release/prepare_bootstrap_inputs.test.mjs > "$work/consumer-restored.log" 2>&1
