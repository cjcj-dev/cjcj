#!/usr/bin/env bash
# Shared eight-payload publisher for GHA and the kkk2 depot.
...
publish_fixed_tuple_to_depot() {
    ...
    (
        cd "$depot" || exit 1
        sha256sum -- "./MANIFEST" "./bin/llc" "./bin/opt" "./lib/STATIC_LLVM.txt" \
            "./fixed-llc/llc.gz" "./fixed-llc/opt.gz" \
