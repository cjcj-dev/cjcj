#!/usr/bin/env python3
"""Link memory-dispatch fixtures to existing product release archives.
...
def main():
    ...
    if record['build_rc'] == 0:
        ...
        diagnostics = {
            'allocate': 'Int64&',
            'load': 'expect 1 operand(s), but there are 0 in fact.',
            'store': 'expect 2 operand(s), but there are 0 in fact.',
            'get-ref': 'expect 1 operand(s), but there are 0 in fact.',
            'get-name': 'you should convert this expression to `GetElementRef`.',
            'store-ref': 'expect 2 operand(s), but there are 0 in fact.',
            'store-name': 'you should convert this expression to `StoreElementRef`.',
            'control': '', 'allocate-good': '', 'load-good': '',
        }
