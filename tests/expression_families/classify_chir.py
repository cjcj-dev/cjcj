    ...
    for index, case in enumerate(manifest['cases']):
        ...
        if not left.exists() or not right.exists():
            ...
            for path, av, bv in diff:
                ...
                elif re.fullmatch(r'/values/\d+/base/identifier', path) and re.fullmatch(r'%\d+', str(av)) and re.fullmatch(r'%\d+', str(bv)):
                    names.append({'path': path, 'baseline': av, 'candidate': bv})
                else:
                    unexpected.append({'path': path, 'baseline': av, 'candidate': bv})
            row.update(status='ran', expected_local_id_differences=expected, expected_local_name_differences=names, unexpected_differences=unexpected,
            ...
            del a, b, diff
            print(f"CHIR_DIFF {case['source']} localId={len(expected)} local_names={len(names)} unexpected={len(unexpected)}", flush=True)
