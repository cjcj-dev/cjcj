...
- Restored the C++ `SipHash::GetHashValue(const char*)` facade with a `CString` overload that hashes bytes up to the first NUL via libc `strlen`/`memcpy` before entering the shared SipHash core.
