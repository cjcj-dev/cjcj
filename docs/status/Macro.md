...
- `packages/macro/cjpm.toml` now depends on `basic` and `lex`; Macro re-exports the real Basic `Position`/`Range`/`SourceManager`/`MacroCallDiagInfo` and real Lex `Token`/`TokenKind` instead of carrying local copies.
