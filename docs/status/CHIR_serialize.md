...
- Added custom type definition records for local and imported structs/classes/enums/extends, generic
  ...
  links, extended types, debug locations, and attributes.
...
- Added function parameter records preserving parameter ids, identifiers, source identifiers, types,
  annotations, debug locations, and attributes.
- Added block group, block, and expression records preserving body ownership, entry blocks, block order,
  landing-pad exception class types, expression order, result locals, operands, terminator successors,
  constant literal payloads, debug locations, and attributes.
- Deserialization is now multi-pass: package/custom defs, globals/functions/parameters, block groups/blocks,
  relationship configuration, expression creation, block-group entry/owner repair, and package function repair.
...
- C++ annotations stored in `Base` are only partially exposed in the current Cangjie CHIR APIs; this pass preserves
  debug locations, attributes, and public `AnnoInfo`, but not the full typed annotation union.
...
Honest coverage estimate for CHIR serializer/deserializer scope: 34%.
