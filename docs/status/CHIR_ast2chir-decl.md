...
- Several C++ CHIR fields do not yet exist in the current self-host IR surface, including full annotation factory linkage, link type info, static member var storage distinct from instance fields, interface lists for struct/enum/extend defs, and deserialization/specific-merge state.
