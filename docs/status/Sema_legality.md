...
- Const-evaluation coverage follows the C++ legality shape but does not yet reproduce every target-specific profile check, string concatenation shortcut, and diagnostic specialization from the C++ implementation.
...
Honest behavior coverage for this legality/const-evaluation scope is about 52% versus the C++ reference. The port now has real traversal and issue production in the scoped files, but production completeness still requires diagnostic integration and the remaining exact semantic edge cases above.
