...
- Added the after-typecheck package declaration pass, including static-initializer call insertion, implicit `return ()` synthesis, final-expression return wrapping, and property annotation cloning for generated accessors.
...
- Added extend-box marking over typed AST expressions, including variable initializers, assignments, calls, conditions, returns, arrays, tuples, try/match/if blocks, and type patterns, plus TypeManager bookkeeping for boxed types and used extend declarations.
