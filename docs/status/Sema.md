...
- Several required upstream/downstream C++ surfaces (`Frontend`, full `Macro`, full `IncrementalCompilation`, and complete `Modules` integration) are not yet available as faithful Cangjie packages, so the affected Sema entrypoints contain `TODO(selfhost:Sema)` markers with compiling conservative bodies.
