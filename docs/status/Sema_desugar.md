...
- In-type-check desugar now has real flow, operator-overload, subscript-overload, `operator()` call, variadic-call, pointer-call, and array-call helpers. Primary constructor lowering, pointer excess-argument diagnostics, cache invalidation through `ASTContext`, and root typechecker wiring remain pending.
