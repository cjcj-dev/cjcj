...
- Deepened `packages/sema/src/Desugar/AfterTypeCheck/EffectHandlers.cj` against the C++ `AfterTypeCheck/EffectHandlers.cpp` expression-level paths:
  ...
  - wires `PERFORM_EXPR` and `RESUME_EXPR` dispatch into the after-type-check traversal while leaving full `try handle` frame construction guarded until the larger `TypeCheckerImpl`/ASTContext-style synthesis hooks are available.
...
- String interpolation now has real `StringBuilder` lowering through package/imported declarations, try-with-resources now has the C++ nested AST shape for resource closing and exception preservation, and immediate effect `perform`/`resume` expressions now lower through real `stdx.effect` declarations. These still lack the C++ `TypeCheckerImpl` synthesis recovery hooks for generated nodes because `ASTContext`/import-manager state is not currently threaded through the self-hosted desugar facade. Full `try handle` frame construction, macro desugar native wrappers, property desugar, Java/ObjC interop branches, main invocation synthesis, and linkage refresh behavior remain below C++ fidelity.
