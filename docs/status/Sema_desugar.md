...
- Deepened `packages/sema/src/Desugar/AfterTypeCheck/TryExpr.cj` against the C++ `AfterTypeCheck/TryExpr.cpp` reference:
  - replaced the status-only body with real try-with-resources lowering that builds the nested `freshExc`, resource declaration, inner try/catch/finally, close guard, and final rethrow match structure used by the C++ pass;
  - uses real AST/sema structures for `Option<Exception>`, `Some`, `None`, `ExceptTypePattern`, `ThrowExpr`, `MatchExpr`, `FieldLookup`, and generated member/call expressions instead of local compatibility copies;
  - wires `TRY_EXPR` dispatch into the after-type-check traversal and clears the original resource/catch/finally lists once the desugared expression is installed, preventing the old resource-spec surface from being walked as active code.
- Deepened `packages/sema/src/Desugar/AfterTypeCheck/EffectHandlers.cj` against the C++ `AfterTypeCheck/EffectHandlers.cpp` expression-level paths:
  - replaced the status-only body with real immediate-effect helpers over the real AST/sema packages, including imported declaration lookup for `stdx.effect.HandlerFrame`, `stdx.effect.ImmediateHandlerReturn`, `std.core.Exception`, and `std.core.Error`;
  - added the C++ `CreateRefValue`/`CreateRefExc`/`CreateRefErr` equivalent for `ImmediateHandlerReturn<T>.Result/Exc/Err`, preserving enum constructor targets and instantiated type arguments;
  - lowers typed `perform cmd` nodes to checked `HandlerFrame.perf<CommandTy, ResultTy>(cmd)` calls and typed `resume` nodes to `return ImmediateHandlerReturn<T>.Result(value)`, `return ...Result(())`, `return ...Exc(exception)`, or `return ...Err(error)` depending on the validated handler metadata;
  - added a guarded `try handle` frame rewrite that builds the `ImmediateFrame<T>` creation, binds the try lambda through `$frameLambda`, wraps immediate handler lambdas in a generated try/catch body that rethrows `ImmediateFrameExceptionWrapper`/`ImmediateFrameErrorWrapper` and normal handler results through `ImmediateEarlyReturn`, registers handlers with `frame.setImmediateHandler`, registers finally lambdas with `frame.setFinally`, and finishes with the C++-style `frame.start()` runtime type match;
  - wires `TRY_EXPR`, `PERFORM_EXPR`, and `RESUME_EXPR` effect-handler dispatch into the after-type-check traversal, clearing the original try-handle surface after the generated frame block is installed so the self-host walker does not process both the source and desugared surfaces.
...
- String interpolation now has real `StringBuilder` lowering through package/imported declarations, try-with-resources now has the C++ nested AST shape for resource closing and exception preservation, and immediate effect handlers now lower through real `stdx.effect` frame/handler/result declarations. These still lack the C++ `TypeCheckerImpl` synthesis recovery hooks for generated nodes because `ASTContext`/import-manager state is not currently threaded through the self-hosted desugar facade. The effect-handler try-lambda enclosure that catches active-frame wrapper exceptions/early returns is still incomplete, and macro desugar native wrappers, property desugar, Java/ObjC interop branches, main invocation synthesis, and linkage refresh behavior remain below C++ fidelity.
...
Real behavior coverage for this scoped desugar area is about 56% versus the C++ reference. The implemented pieces now perform meaningful AST transformations, including in-typecheck operator/variadic/builtin-call and primary-constructor helper lowering, after-type-check option boxing, range and string `for-in` lowering, string interpolation builder lowering, try-with-resources lowering, immediate effect `perform`/`resume` lowering plus guarded `try handle` frame setup, after-instantiation recursive-type/used-import helper wiring, token-call position refresh, quote/macro-declaration lowering, sema-local semantic usage harvesting, the API-level `@IfAvailable` path, LSP macro-call traversal, and more faithful discarded-context branch handling, but substantial C++ behavior remains either not wired into the root pipeline or represented by compiling compatibility bodies.
