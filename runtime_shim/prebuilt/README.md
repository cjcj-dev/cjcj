...
| 平台            | 目录             | 现状                     |
|-----------------|------------------|--------------------------|
| Linux x86_64    | `linux_x86_64`   | ✅ 已提供(CI 构建用)    |
| Linux aarch64   | `linux_aarch64`  | ⬜ 缺(需在补丁版环境产) |
| macOS aarch64   | `darwin_aarch64` | ⬜ 缺                    |
| macOS x86_64    | `darwin_x86_64`  | ⬜ 缺                    |

只有 `linux_x86_64` 存在,故当前 CI/Release **只能在 linux-x64 上构建 cjcj 本体**;其余平台
显式 skip(见 `.github/workflows/*.yml` 注释),绝不放假绿 job。
