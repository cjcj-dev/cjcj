...
| 3 · dry-run + 立即归档 | 同一个 run 的五个平台 package job 执行；publish 必须是 `skipped`。run 一完成立刻按 §3 抓 run/job URL、原始合并日志、五份旁挂 manifest 和五份 checksum 到持久目录，然后运行 `collect` 与 `verify`。 | CI 产包；归档 lane 或主控抓取 | run `status=completed/conclusion=success`；五个 `<platform> / Build release package` 各恰一个且全 `success`；`Publish release` 恰一个且 `skipped`；归档工具输出 `ARCHIVE_EVIDENCE_OK ... manifests=5 checksums=5`。这些检查由 `scripts/archive_release_evidence.mjs:90-151,248-329,363-415` 实施。 | 任一 job 非 success/skipped、五格不齐、publish 未 skipped、log 为空、URL 缺失、manifest/checksum 缺失或工具非 0，就是 `STOP-DRY-RUN`；保留失败 run 的 URL/日志，不得进入抽包复核。 |
