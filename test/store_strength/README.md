...
`run_aggregate.sh` covers the value-record boundary: a String aggregate store,
a direct reference in the same struct, and an ordinary class reference store.
Set `STRENGTH_CJC`, `STRENGTH_OPT`, and `STRENGTH_OUT` with the matching SDK and
loader environment. It compiles `aggregate.cj`, runs the product
`cj-barrier-split` pass on retained frontend IR, and checks each target's emitted
reference access independently. A String's aggregate copy must produce explicit
