...
for match in re.finditer(r"warning: ([^\n]+)\n(?:.*\n){0,3}?.*?fixture\.cj:(\d+):(\d+)", log):
    warnings.append((match.group(1).strip(), int(match.group(2)), int(match.group(3))))
if not warnings:
    for match in re.finditer(r"fixture\.cj:(\d+):(\d+): warning: ([^\n]+)", log):
        warnings.append((match.group(3).strip(), int(match.group(1)), int(match.group(2))))

(out / "warnings.txt").write_text("".join(f"{line}:{col}: {msg}\n" for msg, line, col in warnings))
print("PARSED", len(warnings))
for msg, line, col in warnings:
    print(f"  {line}:{col}: {msg}")

def count(predicate):
    return sum(1 for item in warnings if predicate(*item))
...
outer_hits = count(lambda msg, line, col: "unreachable expression" in msg and line == outer_return)
if profile == "o0":
    expect("outer-return-not-skipped", outer_hits == 1, f"hits={outer_hits} line={outer_return}")

else_if_hits = count(lambda msg, line, col: "unreachable block in 'if'" in msg and line == else_if)
else_arm_hits = count(lambda msg, line, col: "unreachable block in 'if'" in msg and line == else_arm)
...
after_hits = count(lambda msg, line, col: "unreachable expression" in msg and line == after)
expect("return-continuation-warned", after_hits == 1, f"hits={after_hits} line={after}")
later_col = lines[later - 1].index(", 7") + len(", ") + 1
later_hits = count(lambda msg, line, col: "unreachable expression" in msg and line == later and col == later_col)
expect("nothing-later-arg", later_hits == 1, f"hits={later_hits} line={later} col={later_col}")
call_hits = count(lambda msg, line, col: "unreachable 'call'" in msg and line == later)
expect("nothing-call-warning", call_hits == 1, f"hits={call_hits} line={later}")
split_hits = count(lambda msg, line, col: "unreachable" in msg and line == split_case)
constructor_case = line_of("Left(1) | Right(2)")
constructor_hits = count(lambda msg, line, col: "unreachable" in msg and line == constructor_case)
if profile == "o0":
...
reachable_hits = count(lambda msg, line, col: "unreachable" in msg and line == reachable_return)
