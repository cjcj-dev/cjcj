...
link="--export-dynamic --start-group ${libs[*]} --end-group $product/runtime_shim/cjselfhost_llvmshim.o $product/runtime_shim/cjc_runtime_config.o $llvm -lstdc++"
"$CANGJIE_HOME/bin/cjc" --test -O1 "${args[@]}" "$source_dir/ExtensionDefType_test.cj" --link-options "$link" -o "$output"
sha256sum "$output" "$llvm" "${libs[@]}" > "$output.sha256"
